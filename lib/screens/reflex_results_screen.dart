...
class ReflexResultsScreen extends StatelessWidget {
  const ReflexResultsScreen({super.key});

  @override
  Widget build(BuildContext context) {
    final controller = context.watch<ReflexController>();
    final ranked = controller.rankedByAverage;
    final winner = ranked.first;
    final isTie = ranked.length > 1 && ranked[1].averageMs == winner.averageMs;
                ...
                for (final player in ranked)
                  Card(
                    child: ListTile(
                      ...
                      trailing: Text(
                        '${player.averageMs.round()} ms ortalama',
                      ),
                    ),
                  ),
                const SizedBox(height: 24),
