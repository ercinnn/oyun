...
class TownResultsScreen extends StatelessWidget {
  const TownResultsScreen({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    final isTie =
        ranked.length > 1 && ranked[1].totalScore == winner.totalScore;
                ...
                for (final player in ranked)
                  Card(
                    child: ListTile(
                      ...
                      trailing: Text('${player.totalScore} puan'),
                    ),
                  ),
                const SizedBox(height: 24),
                FilledButton(
